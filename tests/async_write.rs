...
async fn test_writer_be() {
    use bitstream_io::{BigEndian, AsyncBitWrite, AsyncBitWriter};

    let final_data: [u8; 4] = [0xB1, 0xED, 0x3B, 0xC1];

    /*writing individual bits*/
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(2), BigEndian);
    ...
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(4), BigEndian);
    ...
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(3), BigEndian);
...
async fn test_writer_edge_cases_be() {
    ...
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(48), BigEndian);
    ...
    {
        AsyncBitWriter::endian(&mut bytes, BigEndian)
...
async fn test_writer_huffman_be() {
    ...
    use bitstream_io::{BigEndian, AsyncBitWrite, AsyncBitWriter, AsyncHuffmanWrite};
...
async fn test_writer_le() {
    use bitstream_io::{AsyncBitWrite, AsyncBitWriter, LittleEndian};

    let final_data: [u8; 4] = [0xB1, 0xED, 0x3B, 0xC1];

    /*writing individual bits*/
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(2), LittleEndian);
    w.write_bit(true).await.unwrap();
    w.write_bit(false).await.unwrap();
    w.write_bit(false).await.unwrap();
    w.write_bit(false).await.unwrap();
    ...
    /*writing unsigned values*/
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(4), LittleEndian);
    assert!(w.byte_aligned());
    ...
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(3), LittleEndian);
    w.write(4, 11u32).await.unwrap();
    w.write_bytes(b"\xB1\xED").await.unwrap();
    w.byte_align().await.unwrap();
    assert_eq!(w.into_writer().as_slice(), &final_data);
...
async fn test_writer_edge_cases_le() {
    ...
    /*unsigned 32 and 64-bit values*/
    let mut w = AsyncBitWriter::endian(Vec::with_capacity(48), LittleEndian);
    w.write(32, 0u32).await.unwrap();
    w.write(32, 4294967295u32).await.unwrap();
    w.write(32, 2147483648u32).await.unwrap();
    w.write(32, 2147483647u32).await.unwrap();
    ...
    let mut bytes = Vec::new();
    {
        AsyncBitWriter::endian(&mut bytes, LittleEndian)
            .write_signed(8, std::i8::MAX)
            .await.unwrap();
    }
...
async fn test_writer_huffman_le() {
    use bitstream_io::huffman::compile_write_tree;
    use bitstream_io::{AsyncBitWrite, AsyncBitWriter, AsyncHuffmanWrite, LittleEndian};
...
async fn test_writer_io_errors_be() {
    ...
    /*individual bits*/
    let mut w = AsyncBitWriter::endian(LimitedWriter::new(1), BigEndian);
    ...
    /*signed values*/
    let mut w = AsyncBitWriter::endian(LimitedWriter::new(1), BigEndian);
    assert!(w.write_signed(2, -2i64).await.is_ok());
    assert!(w.write_signed(3, -2i64).await.is_ok());
    assert!(w.write_signed(5, 7i64).await.is_ok());
    assert!(w.write_signed(3, -3i64).await.is_ok());
    assert_eq!(
        w.write_signed(19, -181311i64).await.unwrap_err().kind(),
        ErrorKind::WriteZero
    );
...
async fn test_writer_io_errors_le() {
    ...
    /*individual bits*/
    let mut w = AsyncBitWriter::endian(LimitedWriter::new(1), LittleEndian);
    assert!(w.write_bit(true).await.is_ok());
    assert!(w.write_bit(false).await.is_ok());
    assert!(w.write_bit(false).await.is_ok());
    assert!(w.write_bit(false).await.is_ok());
    ...
    /*signed values*/
    let mut w = AsyncBitWriter::endian(LimitedWriter::new(1), LittleEndian);
    assert!(w.write_signed(2, 1i64).await.is_ok());
    assert!(w.write_signed(3, -4i64).await.is_ok());
    assert!(w.write_signed(5, 13i64).await.is_ok());
    assert!(w.write_signed(3, 3i64).await.is_ok());
    assert_eq!(
        w.write_signed(19, -128545i64).await.unwrap_err().kind(),
        ErrorKind::WriteZero
    );
    ...
    /*un-aligned bytes*/
    let mut w = AsyncBitWriter::endian(LimitedWriter::new(1), LittleEndian);
    assert!(w.write(4, 11i64).await.is_ok());
    assert_eq!(
        w.write_bytes(b"\xB1\xED").await.unwrap_err().kind(),
        ErrorKind::WriteZero
    );
...
async fn test_writer_bits_errors() {
    use bitstream_io::{BigEndian, AsyncBitWrite, AsyncBitWriter, LittleEndian};
    use std::io::ErrorKind;
    use tokio::io::sink;
    ...
    assert_eq!(
        w.write(1, 0b10u64).await.unwrap_err().kind(),
        ErrorKind::InvalidInput
    );
    assert_eq!(
        w.write(2, 0b100u64).await.unwrap_err().kind(),
        ErrorKind::InvalidInput
    );
    assert_eq!(
        w.write(3, 0b1000u64).await.unwrap_err().kind(),
        ErrorKind::InvalidInput
    );
    ...
    let mut w = AsyncBitWriter::endian(sink(), LittleEndian);
    assert_eq!(w.write(9, 0u8).await.unwrap_err().kind(), ErrorKind::InvalidInput);
    assert_eq!(
        w.write(17, 0u16).await.unwrap_err().kind(),
        ErrorKind::InvalidInput
...
async fn test_counter_be() {
    use bitstream_io::{BigEndian, BitCounter, AsyncBitWrite};
...
async fn test_counter_huffman_be() {
    use bitstream_io::huffman::compile_write_tree;
    use bitstream_io::{BigEndian, BitCounter, AsyncBitWrite, AsyncHuffmanWrite};
...
async fn test_counter_le() {
    use bitstream_io::{BitCounter, AsyncBitWrite, LittleEndian};
...
async fn test_counter_huffman_le() {
    use bitstream_io::huffman::compile_write_tree;
    use bitstream_io::{BitCounter, AsyncBitWrite, AsyncHuffmanWrite, LittleEndian};
...
async fn test_recorder_be() {
    use bitstream_io::{BigEndian, BitRecorder, AsyncBitWrite, AsyncBitWriter};
    ...
    w.write_bit(true).await.unwrap();
    assert_eq!(w.written(), 16);
    let mut w2 = AsyncBitWriter::endian(Vec::with_capacity(2), BigEndian);
    ...
    assert!(w.byte_aligned());
    assert_eq!(w.written(), 32);
    let mut w2 = AsyncBitWriter::endian(Vec::with_capacity(4), BigEndian);
    w.playback(&mut w2).await.unwrap();
    ...
    w.write_bytes(b"\xB1\xED").await.unwrap();
    w.byte_align().await.unwrap();
    let mut w2 = AsyncBitWriter::endian(Vec::with_capacity(3), BigEndian);
    w.playback(&mut w2).await.unwrap();
    assert_eq!(w2.into_writer().as_slice(), &final_data);
}
...
async fn test_recorder_huffman_be() {
    use bitstream_io::huffman::compile_write_tree;
    use bitstream_io::{BigEndian, BitRecorder, AsyncBitWrite, AsyncBitWriter, AsyncHuffmanWrite};
...
async fn test_recorder_le() {
    use bitstream_io::{BitRecorder, AsyncBitWrite, AsyncBitWriter, LittleEndian};
    ...
    w.write_bit(true).await.unwrap();
    assert_eq!(w.written(), 16);
    let mut w2 = AsyncBitWriter::endian(Vec::with_capacity(2), LittleEndian);
    w.playback(&mut w2).await.unwrap();
    assert_eq!(w2.into_writer().as_slice(), &final_data[0..2]);
    ...
    assert!(w.byte_aligned());
    assert_eq!(w.written(), 32);
    let mut w2 = AsyncBitWriter::endian(Vec::with_capacity(4), LittleEndian);
    w.playback(&mut w2).await.unwrap();
    assert_eq!(w2.into_writer().as_slice(), &final_data);
    ...
    w.byte_align().await.unwrap();
    assert_eq!(w.written(), 24);
    let mut w2 = AsyncBitWriter::endian(Vec::with_capacity(3), LittleEndian);
    w.playback(&mut w2).await.unwrap();
    assert_eq!(w2.into_writer().as_slice(), &final_data);
}
...
async fn test_recorder_huffman_le() {
    use bitstream_io::huffman::compile_write_tree;
    use bitstream_io::{BitRecorder, AsyncBitWrite, AsyncBitWriter, AsyncHuffmanWrite, LittleEndian};
